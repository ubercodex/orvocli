...
**What this prevents:**
- ❌ `https://zalcli.com/server/.env` → 403 Forbidden
- ❌ `https://zalcli.com/server/data/registry.db` → 403 Forbidden
- ❌ `https://zalcli.com/deploy/deploy.sh` → 403 Forbidden
- ❌ `https://zalcli.com/.git/config` → 403 Forbidden

### 2. **File System Permissions**

```bash
# .env files - root only
chmod 600 /var/www/zalcli-registry/server/.env

# Database directory - root only
chmod 700 /var/www/zalcli-registry/server/data/

# Source code - root only
chmod 600 /var/www/zalcli-registry/server/src/*

# Public files - world readable
chmod 755 /var/www/zalcli-registry/client/dist/
```
...
SQLite database is stored in `/var/www/zalcli-registry/server/data/registry.db`
...
```bash
curl https://zalcli.com/server/.env
...
```bash
curl https://zalcli.com/server/data/registry.db
...
```bash
# Backup database
cp /var/www/zalcli-registry/server/data/registry.db ~/backups/registry-$(date +%Y%m%d).db
```
...
If you find a security vulnerability, please email: security@zalcli.com
